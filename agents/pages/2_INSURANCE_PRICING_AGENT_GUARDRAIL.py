...
# ========== GUARDRAIL PARAMETER ==========
MAX_PREMIUM_PCT_OF_REVENUE = 0.05  # 5% of company revenue
# ========================================
...
print(f"  • Revenue Guardrail: Max {MAX_PREMIUM_PCT_OF_REVENUE * 100}% of company revenue")
...
def calculate_pure_premium(
    ...
    """
    Calculate pure premium with guardrail: max 5% of company revenue.
    
    Args:
        frequency: Predicted incident frequency
        severity: Expected loss severity
        company_revenue: Annual company revenue in USD
        
    Returns:
        dict: Premium breakdown with guardrail applied if necessary
    """
    ...
    # Apply 5% of revenue guardrail
    max_allowable_premium = company_revenue * MAX_PREMIUM_PCT_OF_REVENUE
    is_guardrail_applied = False
    guardrail_reduction_factor = 1.0
    
    if final_premium > max_allowable_premium:
        is_guardrail_applied = True
        guardrail_reduction_factor = max_allowable_premium / final_premium
        
        # Scale down all components proportionally
        pure_premium = pure_premium * guardrail_reduction_factor
        total_loading = total_loading * guardrail_reduction_factor
        loading_dict = {k: v * guardrail_reduction_factor for k, v in loading_dict.items()}
    ...
    return {
        ...
        "guardrail_applied": is_guardrail_applied,
        "guardrail_max_premium": round(max_allowable_premium, 0),
        "guardrail_reduction_factor": round(guardrail_reduction_factor, 4),
    }
...
print("✓ Prediction functions defined with EXTRACTED coefficients and guardrail")
...
def premium_quotation_tool(
    ...
    """Generate premium quote using extracted model coefficients with revenue guardrail"""
    try:
        ...
        # Pass company_revenue to calculate_pure_premium for guardrail application
        premium_calc = calculate_pure_premium(
            frequency, 
            severity, 
            company_revenue_usd
        )
        ...
        # Prepare guardrail notification
        guardrail_note = ""
        if premium_calc["guardrail_applied"]:
            guardrail_note = (
                f"\n⚠️  GUARDRAIL APPLIED: Premium capped at 5% of revenue (${premium_calc['guardrail_max_premium']:,.0f}). "
                f"Reduction factor: {premium_calc['guardrail_reduction_factor']:.2%}"
            )
        ...
        quotation = {
            ...
            "guardrail_information": {
                "max_premium_pct_of_revenue": f"{MAX_PREMIUM_PCT_OF_REVENUE * 100}%",
                "max_allowable_premium": f"${premium_calc['guardrail_max_premium']:,.0f}",
                "guardrail_applied": premium_calc["guardrail_applied"],
                "guardrail_note": guardrail_note if guardrail_note else "No guardrail applied - premium within acceptable limits"
            },
...
print("✓ Agent tools defined with guardrail logic")
...
def create_quotation_agent() -> Agent:
    """Create agent with extracted model coefficients and guardrail enforcement"""
    # tools = [
    #     {
...
- GUARDRAIL: Premium capped at maximum 5% of company annual revenue
...
4. Highlight any guardrail application and explain implications
5. Recommend appropriate coverage based on company profile:
   - Small/Medium: Tier 1
...
8. If any of the numeric inputs (revenue, employees, data records) entered is a non-positive number, then inform the user about the mistake and do not generate the quote or provide a risk warning.
...
print("✓ Agentic AI agent created and ready (with guardrail enforcement)")
...
print(f"\n  GUARDRAIL ENGINE:")
print(f"    Max Premium Cap: {MAX_PREMIUM_PCT_OF_REVENUE * 100}% of company revenue")
print(f"    Protection Level: Enterprise risk mitigation")
...
# Pass company_revenue to calculate_pure_premium
test_premium = calculate_pure_premium(
    test_freq["predicted_frequency"], 
    test_sev["expected_severity"],
    150e9  # company_revenue
)
...
print(f"\nGuardrail Status:")
print(f"  • Max Allowable Premium (5% of revenue): ${test_premium['guardrail_max_premium']:,.0f}")
print(f"  • Guardrail Applied: {test_premium['guardrail_applied']}")
if test_premium['guardrail_applied']:
    print(f"  • Reduction Factor: {test_premium['guardrail_reduction_factor']:.2%}")
...
print(f"\n✓ All functions working correctly with EXTRACTED coefficients and guardrail!")
