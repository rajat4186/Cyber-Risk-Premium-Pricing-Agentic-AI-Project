...
# 2_INSURANCE_PRICING_AGENT_GUARDRAIL.py / 2_INSURANCE_PRICING_AGENT_FIXED.py,
# where the revenue_tier coefficient is intentionally dropped from the
# coefficients dict and omitted from the log_lambda calculation.
# ============================================================================

_FREQ_FALLBACK = {
    "intercept": -1.4502, "log_revenue": 0.0841, "log_employees": 0.0512,
    "is_public": 0.1292
    # "revenue_tier" intentionally omitted - aligned with GUARDRAIL/FIXED insurance agents
}
...
def train_models():
    ...
    try:
        ...
        # Model is still fit with revenue_tier included (matches GUARDRAIL/FIXED
        # insurance agents), but the coefficient is not extracted/used below.
        ...
        freq_coefs = {
            ...
            # "revenue_tier": float(freq_model.coef_[3]),  # dropped - aligned with GUARDRAIL/FIXED
        }
...
# / 2_INSURANCE_PRICING_AGENT_FIXED.py, where the revenue-tier relativity
# multiplication was commented out (adjusted_premium = base_final_premium * industry_rel).
...
# ============================================================================
# PHASE 1.5: REINSURANCE PRICING ENGINE
# (Ported from 3_REINSURANCE_PRICING_AGENT_FIXED.py, including the corrected
# base-rate scaling fix: mean_final_premium / 1_000_000_000 instead of / 1_000)
# ============================================================================

@st.cache_data(show_spinner="Calibrating reinsurance premium statistics...")
...
def calculate_gross_premium(company_revenue: float, industry_code: str) -> Dict:
    ...
    FIX applied (from 3_REINSURANCE_PRICING_AGENT_FIXED.py): base_rate divides
    mean_final_premium by 1,000,000,000 (not 1,000) to correct the rate scale.
    """
    lookup_code = industry_code if industry_code in INDUSTRY_DATA else '54'
    relativity = INDUSTRY_DATA[lookup_code]['relativity']
...
print("✓ Reinsurance pricing engine loaded (aligned with REINSURANCE_PRICING_AGENT_FIXED)")
...
def calculate_insurance_quotation(revenue: float, employees: int, is_public: bool, industry_code: str) -> dict:
    ...
    Sync updates applied here:
      ...
      2. Revenue-tier relativity is no longer multiplied into the final
         ...
         GUARDRAIL/FIXED insurance agents.
      3. Reinsurance allocation now uses the actual quota-share / surplus-share
         / excess-of-loss tower math from 3_REINSURANCE_PRICING_AGENT_FIXED.py
         (including the corrected 1e9 base-rate scaling), instead of the
         previous placeholder approximation. Output dict keys are unchanged
         so the narrative builder, PDF report, and Streamlit tabs continue to
    ...
    final_premium = base_final_premium * ind_rel  # * rev_rel  -- removed, aligned with GUARDRAIL/FIXED

    # ---- Reinsurance allocation (ported from REINSURANCE_PRICING_AGENT_FIXED) ----
    reins_gross_premium = calculate_gross_premium(revenue, industry_code)['gross_premium_usd']
    proportional = calculate_proportional_reinsurance(reins_gross_premium)
    xol = calculate_xol_reinsurance(reins_gross_premium)
