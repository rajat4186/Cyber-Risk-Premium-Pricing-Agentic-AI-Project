...
def generate_pdf_report(quotation_data_json: str, output_filename: str = "Cyber_Risk_Insurance_Report.pdf") -> str:
    ...
    premium_table = [
        [Paragraph("<b>Premium Component Matrix</b>", body_style), Paragraph("<b>Allocation Amount (USD)</b>", body_style)],
    ...
    premium_table[0][0].style.textColor = colors.white
    premium_table[0][1].style.textColor = colors.white
    story.append(t2)
    story.append(Spacer(1, 12))
...
def run_integrated_pipeline_app():
    ...
    with t_metrics:
        st.json(active_data["risk_metrics"])
    with t_premium:
        st.json(active_data["premium_calculations"])
    with t_reinsurance:
        st.json(active_data["reinsurance_allocation"])
